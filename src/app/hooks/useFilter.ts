...
export default function useFilter(filter: string) {
	...
	const logRenderer = useMemo(
		() => ctxLogRenderer ?? (async (logs: LogData[]) => logs.map(() => ({}))),
		[ctxLogRenderer]
	);
	...
	const setColumns = useCallback(
		async (newColumns: ColumnData[]) => {
			dispatch({ type: 'setColumns', filter, columns: newColumns });
			dispatch({
				type: 'setLogRenderer',
				filter,
				logRenderer: await sandbox.createLogRenderer(newColumns)
			});
		},
	...
	const setColumn = useCallback(
		async (id: string, partialData: Partial<ColumnData> | null) => {
	...
	const setConfig = useCallback(
		async (name: string, filterString: string) => {
			const filterFunction = (await sandbox.createCallback(filterString)) as (
				log: LogData[]
			) => Promise<boolean[]>;
