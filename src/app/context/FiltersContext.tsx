...
export const FiltersContext = createContext<
	[FiltersContextType, Dispatch<FiltersReducerActionType>]
>([initialContext, () => {}]);
...
const filtersReducer: Reducer<FiltersContextType, FiltersReducerActionType> = (ctx, action) => {
	...
	switch (action.type) {
		...
		case 'addLogs': {
			...
			ctxCopy.filters.set(action.filter, {
				...filter,
				logs: [...(filter?.logs ?? []), ...action.logs]
			});
			return ctxCopy;
		}
		case 'setLogs': {
...
export function FiltersProvider({ children }: FiltersProviderProps) {
	...
	return <FiltersContext.Provider value={[context, dispatch]}>{children}</FiltersContext.Provider>;
}
