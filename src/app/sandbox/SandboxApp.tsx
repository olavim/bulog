...
${code}`;
    ...
    const callFunction = useCallback(async (fn: JSONFunc, args: JSONValue[]): Promise<Array<JSONValue | Error>> => {
    ...
    const callFunctionGroup = useCallback(async (group: { [id: string]: JSONFunc }, args: JSONValue[]) => {
        return Promise.all(args.map(async arg => {
    ...
    onMessage(async evt => {
        ...
        if (evt.data.type === 'create-fn-group') {
            ...
            functionGroups.current[id] = Object.keys(codes).reduce((obj, key) => ({ ...obj, [key]: createFunction(codes[key]) }), {});
